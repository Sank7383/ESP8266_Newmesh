...
| 1 | Toilet ID | `myConfig.toiletid` | Used to derive `bedToiletShareUnit()` — `true` when `toiletid == machineid`, which gates whether a toilet-call is blocked while the bed already has an active call (see §6). |
